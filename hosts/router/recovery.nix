    ...
    text = ''
      ...
      choose_interface() {
        ...
        while true; do
          ...
          ethtool --identify "$interface" 10 || echo "This port cannot blink" >&2
      ...
      lan $lan_mac
      EOF
      chmod 0644 /mnt/etc/router-interfaces

      install -d -m0755 /mnt/etc/systemd/network
...
in {
  ...
  environment.etc = {
    ...
    issue.text = ''
      ...
      Copy the router SSH host key to /root/router-ssh-host-key, then run:
