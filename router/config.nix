...
in
{
  ...
  config.systemd.services.router-interface-names = {
    ...
    script = ''
      ...
      rename_interface() {
        ...
        for path in /sys/class/net/*; do
          interface="''${path##*/}"
          if [[ "$(${pkgs.coreutils}/bin/cat "$path/address")" == "$mac" ]]; then
      ...
      rename_interface wan wan0
      rename_interface lan lan0
    '';
  };
